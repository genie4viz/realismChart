...
const BarChart = () => {
    ...
    useEffect(() => {
        let x = d3.scaleBand().rangeRound([margin, w - margin]).padding(0.1),
        ...
        try {
          ...
          ioClient.on("data", msg => {
              if(msg.value > thredholdRef.current)
                message.warning(`BarChart : The received number from server ${msg.value} is greater than thredhold ${thredholdRef.current}`);
  ...
  return (
    <div style={{width: '100%', textAlign: 'center'}}>
        <div>
            <Input addonBefore="Alert threshold" addonAfter="" placeholder="Input number..." defaultValue="" onChange={(e) => onChange(e.target.value)} style={{width: '20%'}}/>
        </div>
        <div>
